# Sample Plugin Developer Guide

本文件面向 plugin 開發者，整理 Sample Plugin 的架構、必備功能、開發注意事項與移植自定義 plugin 時應保留的通用規則。一般使用與建置方式請先看 `README.md`；本文件補充更細的實作規範。
...
- `plugins/sample/plugin.json`：主服務讀取的 manifest，定義 plugin id、UI、service entry、routes、runtime hooks 與 capabilities。
- `plugins/sample/config.default.json`：部署用預設設定。runtime 實際使用 `config.json`，部署包不應覆蓋使用者既有 `config.json`。
- `plugins/sample/skill/guide/SKILL.md`：專屬 Skill 對話的 system guide。
- `plugins/sample/skill/skill-cards.default.json`：對話 Skill 卡片預設值。
- `website/sample/index.html`：plugin UI。必須透過主服務 gateway 呼叫 API。
- `src/sampleplugin/http_api.go`：plugin HTTP API、runtime 狀態、config、catalog、定期任務等主要實作。
- `src/sampleplugin/mcp_server.go`：標準無狀態 MCP JSON-RPC transport、版本協商與方法 dispatch。
- `src/sampleplugin/mcp_tools.go`：MCP 工具 Schema、annotations、權限 metadata 與實際操作。
- `src/sampleplugin/service/main.go`：service 啟動入口，負責定位 plugin 根目錄、設定 config path、啟動 HTTP server。

## 原始碼分層建議

Sample 為了示範與封裝方便，目前多數後端邏輯集中在 `src/sampleplugin/http_api.go`。自定義 plugin 成長後，建議依功能與職責分門別類放在 `src` 下的對應目錄，避免單一檔案過大，後續不好管理、測試與拆分。
...
Sample 目前為了方便展示，將 HTML、CSS、JavaScript 集中在 `website/sample/index.html`。自定義 plugin 成長後，建議依功能拆分 `website` 下的前端檔案，避免單一 HTML 過大，後續不好維護、重用與拆分。
...
- API catalog：後端必須提供查詢所有 API 的 catalog。Sample 使用 `GET /api/sample/apis` 與 `GET /api/sample`。
- `GET /api/hello`：最小握手 API，回傳 plugin id、name、version、time。
- `GET /api/health`：健康檢查 API，回傳 service 健康狀態與 plugin runtime 狀態。Sample 另保留 `GET /api/heatlth` 作為拼字相容別名。
...
- `id` 必須穩定，前端 `fetchPlugin("sample", ...)`、後端路由、資料目錄都會依賴它。
...
- `routes` 必須包含所有主服務要 gateway 的根路徑，例如 `/api/sample`、`/api/hello`、`/api/health`。
- `runtime.health` 建議指向 `/api/health`，詳細 runtime 狀態則由 `/api/sample/plugin/status` 回報。
- `runtime.auth`、`load`、`unload`、`registration` 要與後端實作一致。
- `runtime.preserve_paths` 要列出更新套件時不可覆蓋的 runtime 資料；Sample 保留 `config.json`、`runtime/` 與 `skill/skill-cards.json`。
...
- 每張卡的 `website_path` 都必須指向部署包內存在的 HTML。若卡片共用同一 HTML，可透過 query string 或 hash 分流，但文件要寫清楚，例如 `/sample/index.html#ops`。
...
- `/api/hello`：最小握手。
- `/api/health`：健康檢查。
- `/api/sample/apis`：API catalog，必須包含 `path`、`method`、`description`、`examples`，有 request body 的 API 應提供 `request_body`。
- `/api/sample/plugin/status`：runtime 狀態，包含 `loaded`、`loaded_at`、`last_error`、`host_auth` 等。
- `/api/sample/plugin/load`：載入 config、初始化 runtime 狀態、啟動必要背景流程。
- `/api/sample/plugin/unload`：清除 runtime 狀態、停止背景流程，並標記 service pending stop。HTTP response 寫回後才非同步關閉 service process。
- `/api/sample/plugin/reload`：重新載入 config。
- `/api/sample/plugin/registration`：回傳 manifest 類 metadata。

新增 API 時要同步更新：

- `Process` route switch。
- `sampleAPICatalog()`。
- `sampleMCPToolDefinitions()` 與 MCP 執行 switch，如果該能力要讓主服務工具化使用。
- `plugins/sample/README.md`、`README.md` 或 `developer.md`，視 API 重要性更新。
- 前端呼叫點與錯誤處理。
...
Sample 採用 AccountManager 同款流程：

- `HttpAPI_Plugin` 保存 `Shutdown func()` 與 `pendingStop`。
- `/api/sample/plugin/unload` 在 mutex 內清除 runtime 狀態、停止背景流程，並設定 `pendingStop = true`。
- `ServeHTTP` 先把 JSON response 寫回；若 consume 到 pending stop，再 flush response 並非同步呼叫 `Shutdown`。
- `src/sampleplugin/service/main.go` 使用具名 `http.Server`，shutdown 時先 `server.SetKeepAlivesEnabled(false)`，再以 1 秒 timeout 呼叫 `server.Shutdown(ctx)`，失敗才 fallback `server.Close()`。
- 不在 handler 內直接 `os.Exit`，也不在 response 尚未完成前關 listener。

自定義 plugin 若有背景 goroutine、ticker、DB connection 或檔案 handle，必須在 `unload` 設定 pending stop 前先釋放，確保 service process 結束時不再持有 runtime 資源。

## 打包規範

Sample 的打包流程以 `build.sh` 為唯一主流程：

- 建立乾淨 staging 目錄。
- 複製 `plugins/sample`、`website/sample` 與根目錄 `README.md`。
...
- 正式環境呼叫 plugin API 時使用 `window.AgenticTalkAPI.fetchPlugin("sample", "/api/sample/...")`。
- lifecycle 控制使用 `window.AgenticTalkAPI.fetchPluginControl("sample", "/status|/load|/unload|/reload")`。
- 若獨立開發環境沒有 `AgenticTalkAPI`，可以 fallback 到同源 API，但正式文件仍應建議 gateway。
- 需要認證時，沿用主服務注入的 auth headers；Sample 也會從 cookie 補 `Authentication` 與 `Authorization`。
...
- `/api/sample/plugin/load`
- `/api/sample/plugin/reload`

不要用一般 API 回應推導載入狀態。像 `/api/sample/items`、`/api/sample/jobs`、`/api/sample/scheduled-tasks` 這些 API 不一定包含 `plugin.loaded`，若用 `Boolean(data.plugin?.loaded)` 會把缺欄位誤判成 `false`。
...
- 主服務啟動或 reload 後呼叫 `/api/sample/plugin/auth`。
- 後端只把 token 保存在記憶體，不寫入 config。
- 狀態 API 只能回報 `host_auth.available`、來源與時間，不可回傳 token 本文。

## 標準 MCP 設計

Sample 的 `/mcp` 是標準無狀態 JSON-RPC 端點，不是舊式 metadata GET API。前端需要盤點工具時呼叫 `GET /api/sample/mcp`；AgenticService 會依 manifest `mcp_url` 直接探索 Plugin service。
...
- `_meta["io.agenticservice/permission"]` 對應 manifest `permission_settings.items[].key`；Sample 使用 `mcp_read`、`mcp_write`、`mcp_delete`。
...
定期任務是可選能力。Sample 的實作重點：

- 任務資料存在 `config.json` 的 `scheduled_tasks`。
- 執行 LOG 依日期寫入 `plugins/sample/runtime/scheduled-logs/YYYY-MM-DD.jsonl`。
- `enabled` 控制是否排程。
- `interval_minutes` 限制在合理範圍內，Sample 上限為 1440。
- `next_run_at`、`last_run_at`、`running_until`、`run_token` 用於排程與防重入。
- 手動執行使用 `/api/sample/scheduled-tasks/{id}/run`。
- LOG 讀取使用 `/api/sample/scheduled-logs?date=YYYY-MM-DD&task_id=TASK_ID`，前端應提供日期選擇、LOG 數量與上一則/下一則切換。
...
- zip 內包含 `plugins/sample`、`website/sample`、service binaries 與 `build-info.json`。
- 不要把 `config.json`、`skill-cards.json`、runtime 目錄包進部署包。
- 更新 Go 程式後，主服務實際使用的是已編譯 binary；只改原始碼不會影響已匯入的舊 binary。
...
```bash
go test ./...
node -e "JSON.parse(require('fs').readFileSync('plugins/sample/plugin.json','utf8')); JSON.parse(require('fs').readFileSync('plugins/sample/config.default.json','utf8'));"
```

若修改 `website/sample/index.html` 內嵌 script，可額外用 Node 解析 script 語法。除非有特別要求，不需要啟動 service 或打 API；實際功能驗證可由使用者執行。
...
- 修改 Go 原始碼後沒有重新建置 binary。
- 修改可工具化能力但忘記同步 MCP tools/list、執行 switch、權限設定與文件。
